...
func TestLRUCacheLookup(t *testing.T) {
	...
	for _, tc := range testcases {
		t.Run(tc.description, func(t *testing.T) {
			...
			for _, q := range tc.clientInput {
				if err := informerFactory.Core().V1().ResourceQuotas().Informer().GetStore().Add(q); err != nil {
					t.Fatalf("Add to informer: %v", err)
				}
			}
			...
			for _, q := range tc.cacheInput {
				liveLookupCache.Add(q.Namespace, liveLookupEntry{expiry: time.Now().Add(tc.ttl), items: []*corev1.ResourceQuota{q}})
			}
