...
func (ssc *StatefulSetController) watchDelay(rv1, resource string) {
	...
	rv, err := strconv.Atoi(rv1)
	if err != nil {
		klog.ErrorS(err, "DUPA watchAtoi Atoi error")
		return
	}
	ssc.lock.Lock()
...
func (ssc *StatefulSetController) getPodsForStatefulSet(ctx context.Context, set *apps.StatefulSet, selector labels.Selector) ([]*v1.Pod, string, error) {
	// List all pods to include the pods that don't match the selector anymore but
	// has a ControllerRef pointing to this StatefulSet.
	pods, rv, err := ssc.podLister.Pods(set.Namespace).ListRV(labels.Everything())
	if err != nil {
		return nil, rv, err
	}
	...
	pods, err = cm.ClaimPods(ctx, pods, filter)
	return pods, rv, err
}
